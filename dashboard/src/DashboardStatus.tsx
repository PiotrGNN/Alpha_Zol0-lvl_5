...
const DashboardStatus: React.FC = () => {
  ...
  // Local state for start-live feedback
  ...
  // Handler to start live trading via API
  const handleStartLive = () => {
    setLiveMsg(null);
    fetch(`${API_URL}/start-live`, { method: 'POST' })
      .then(res => (res.ok ? res.json() : Promise.reject(res)))
      .then(data => {
        if (data && data.status === 'started') {
          setLiveMsg('Live trading uruchomione');
        } else {
          setLiveMsg(data.error || 'Błąd uruchamiania');
        }
        setTimeout(() => setLiveMsg(null), 5000);
      })
      .catch(() => {
        setLiveMsg('Błąd uruchamiania');
        setTimeout(() => setLiveMsg(null), 5000);
      });
  };
  ...
  return (
    <div style={{ padding: 24, color: '#fff', background: '#181c24', minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <button onClick={handleStartLive} style={{ marginLeft: 16, padding: '4px 12px' }}>
          Start Live Trading
        </button>
      </div>
      {liveMsg && (
        <div style={{ background: '#333', padding: 8, borderRadius: 4, marginBottom: 8 }}>
          {liveMsg}
        </div>
      )}
      ...
      {metricsData && metricsData.length > 0 ? (
        <TrendVolatilityChartAdapter
          data={metricsData.slice(-30).map(m => ({
      ...
      {performanceData && performanceData.pnl !== undefined && metricsData && metricsData.length > 0 ? (
        <PnLChart
          data={metricsData.slice(-30)
            .filter(m => typeof m.volatility === 'number')
            .map(m => ({
              timestamp: m.timestamp,
